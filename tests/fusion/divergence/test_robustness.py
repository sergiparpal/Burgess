...
the KG_DIVERGE_DEBUG truthiness contract, the embedding-dimension guard, the
...
from kg_engine.divergence.archive import Archive, continuous_bin
from kg_engine.divergence.config import (
    Axis,
    Candidate,
    ConfigError,
    Niche,
...
from kg_engine.divergence.originality import originality_scores
...
def test_file_lock_is_mutually_exclusive(home):
    from kg_engine.divergence.state import _file_lock
...
# --------------------------------------------------------------------------- #
# M7: KG_DIVERGE_DEBUG truthiness contract
# --------------------------------------------------------------------------- #
...
def test_debug_enabled(monkeypatch, value, expected):
    monkeypatch.setenv("KG_DIVERGE_DEBUG", value)
    assert debug_enabled() is expected


def test_debug_enabled_unset(monkeypatch):
    monkeypatch.delenv("KG_DIVERGE_DEBUG", raising=False)
