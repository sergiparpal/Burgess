...
Burgess adaptation (I10, session-ephemeral geometry): init-project additionally
begins/resumes a SESSION. Re-init tests that exercise the axes-comparison path
pass the same explicit ``session=`` id (the resume path); omitting it starts a
fresh session, which wipes geometry by design — asserted by
``test_new_session_wipes_geometry_keeps_pins``.
"""
...
from kg_engine.divergence import pipeline, selftest
...
def test_reinit_same_axes_preserves_archive(home):
    ...
    # identical axes, SAME session (the resume path)
    res = pipeline.init_project("p", axes, seed=1, home=home, session="s1")
    assert res["reset"] is False and res["new_session"] is False
...
def test_reinit_changed_axes_resets_geometry_keeps_memory(home):
    ...
    res = pipeline.init_project("p", axes2, seed=1, home=home, session="s1")
...
def test_post_reset_ingest_has_only_new_axis_keys(home):
    ...
    # same session, so the wipe under test is the AXES reset, not the session wipe
    pipeline.init_project("p", axes2, seed=1, home=home, session="s1")
    pipeline.ingest(
...
def test_new_session_wipes_geometry_keeps_pins(home):
    """I10 at the init surface: a NEW session id (or none) wipes the archive zone;
    pins and the axes snapshot survive; the geometry-coupled meta series drop."""
    axes = _axes()
    pipeline.init_project("p", axes, seed=1, home=home, session="s1")
    pipeline.ingest("p", _cands(4), axes, seed=1, home=home)
    pipeline.remember("p", {"type": "pin", "id": "c0"}, home=home)
    st = State("p", home=home)
    assert st.read_archive().get("niches")
    assert st.read_meta().get("cycles", 0) >= 1

    res = pipeline.init_project("p", axes, seed=1, home=home, session="s2")
    assert res["new_session"] is True and res["reset"] is False

    st = State("p", home=home)
    assert st.read_archive() == {} and st.read_candidates() == {}
    assert st.read_embeddings() == {} and st.read_mech_embeddings() == {}
    assert st.read_open_nicher() is None
    assert "c0" in st.read_pins("d")          # durable negative/positive memory survives
    assert st.read_axes() is not None          # axes snapshot survives
    meta = st.read_meta()
    assert "cycles" not in meta and "cos_window" not in meta
    assert st.read_session().get("session_id") == "s2"

    # omitting session= is also a fresh session (auto id): geometry cannot leak
    pipeline.ingest("p", _cands(3), axes, seed=1, home=home)
    assert State("p", home=home).read_archive().get("niches")
    res2 = pipeline.init_project("p", axes, seed=1, home=home)
    assert res2["new_session"] is True
    assert State("p", home=home).read_archive() == {}
